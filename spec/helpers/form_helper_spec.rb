  ...
  describe "policy_data" do
    ...
    describe "starts-with $key" do
      it "is configurable with the key_starts_with option" do
        ...
        expect(s3_uploader.policy_data[:conditions]).to include ["starts-with", "$key", key_starts_with]
      end

      it "defaults to 'uploads/'" do
        s3_uploader = S3DirectUpload::UploadHelper::S3Uploader.new({})
        expect(s3_uploader.policy_data[:conditions]).to include ["starts-with", "$key", "uploads/"]
      end
    end
    ...
    describe "starts-with $content-type" do
      it "is configurable with the content_type_starts_with option" do
        ...
        expect(s3_uploader.policy_data[:conditions]).to include ["starts-with", "$content-type", content_type_starts_with]
      end

      it "is defaults to an empty string" do
        s3_uploader = S3DirectUpload::UploadHelper::S3Uploader.new({})
        expect(s3_uploader.policy_data[:conditions]).to include ["starts-with", "$content-type", ""]
      end
    end
  end
